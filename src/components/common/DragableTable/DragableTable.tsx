...
const DraggableTable: React.FC<DraggableTableProps> = ({ data, loading }) => {
  // State to manage table rows
  const [rows, setRows] = useState<TableRow[] | null>(data);
      ...
      <table className="w-full rounded-xl border-collapse border border-gray-300">
        ...
        <tbody>
          {loading ? (
            ...
            <tr>
              <td colSpan={5} className="h-64 text-center text-gray-500">
                No data available
              </td>
            </tr>
          )}
